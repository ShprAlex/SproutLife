...
A slight change to the rules of GOL allows us to implement both reproduction and mutation. While Conway's Game of Life settles down without achieving reproduction, other cellular automata do create reproducing patterns. When patterns reproduce it's often not clear at which step the child begins and when the parent ends. Sprout Life clarifies this by defining specific Seed and Sprout patterns that let us track the transition. Knowing exactly when a child splits off from a parent lets us determine its age. We can then add mutations to vary how it develops at each step and let those mutations to be inherited from parent to child.
